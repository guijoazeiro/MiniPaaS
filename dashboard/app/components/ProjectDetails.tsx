...
export function ProjectDetails({ name }: { name: string }) {
  ...
  async function deleteApp() {
    setDeletingApp(true);
    try {
      await request(`/apps/${encodeURIComponent(name)}`, { method: "DELETE" });
      await refreshApps();
      setFeedback(`Aplicação ${name} excluída.`);
      router.replace("/dashboard/projects");
    } catch (cause) {
      setFeedback(cause instanceof Error ? cause.message : "Não foi possível excluir a aplicação.", "error");
    } finally {
